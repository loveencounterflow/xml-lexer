...
(function() {
  'use strict';
  var Action, EventEmitter, State, Type, action_from_chr, charToAction, create, isa, noop, type_of, validate,
    indexOf = [].indexOf;
  ...
  action_from_chr = (chr) => {
    var ref;
    return (ref = charToAction[chr]) != null ? ref : Action.chr;
  };
  ...
  create = function(settings, handler) {
    ...
    //---------------------------------------------------------------------------------------------------------
    step = (chr) => {
      var action, actions, ref, ref1;
      if (settings.debug) {
        console.log(state, chr);
      }
      actions = lexer.stateMachine[state];
      action = (ref = (ref1 = actions[action_from_chr(chr)]) != null ? ref1 : actions[Action.error]) != null ? ref : actions[Action.chr];
      action(chr);
      return null;
    };
    //---------------------------------------------------------------------------------------------------------
    ...
    //---------------------------------------------------------------------------------------------------------
    emit = (type, value) => {
      var event, first_chr, ref;
      // tags like: '?xml', '!DOCTYPE', comments
      if (ref = (first_chr = tagName[0]), indexOf.call('?!', ref) >= 0) {
        return null;
      }
      event = {type, value};
      if (settings.debug) {
